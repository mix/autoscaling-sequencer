...
from sequencer.aws.utils import get_instance_id
...
@click.option('-z', '--zookeeper-connect', metavar='', type=click.STRING, help='Zookeeper Connection String',
              required=True)
@click.option('-a', '--autoscaling-group', metavar='', type=click.STRING, help='Autoscaling Group Name', required=True)
...
@click.option('-i', '--instance-id', type=click.STRING, default=get_instance_id, metavar='',
...
def generate(zookeeper_connect, autoscaling_group, strategy, instance_id, max_id_range, aws_region):
    sequence_generator = sequencer.Sequencer(zookeeper_connect, autoscaling_group, strategy, instance_id,
                                             max_id_range, aws_region)
